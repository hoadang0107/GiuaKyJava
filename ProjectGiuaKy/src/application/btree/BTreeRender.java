...
public class BTreeRender extends JFrame {
	...
	public final static int APP_HEIGHT = 500;//650
	...
	private final BTreeIS mTreeTest;
	...
	private final JButton mClearBt, mSearchKeyBt,mBackbt;
	private final JTextArea mOutputConsole;

	public BTreeRender() {
		super("BTree Renderer");
		mTreeTest = new BTreeIS();
		mBuf = new StringBuilder();
		mBackbt = new JButton("Back");
		mObjLists = new Object[TREE_HEIGHT];
		mSearchKeyBt = new JButton("Search");
		mAddBt = new JButton("Insert");
		mRemoveBt = new JButton("Delete");
		...
		try {
			...
			for (int i = 0; i < mObjLists.length; ++i) {
				...
				for (KeyData keyData : objList) {
					...
					cellWidth = len * 6;
					Object gObj = mGraph.insertVertex(parent, null, keyData.mKeys, nStartXPos, nStartYPos, cellWidth,
							24);
					cObjList.add(gObj);
	...
	public void addClickHandler(mxGraphComponent graphComponent) {
		...
		graphComponent.getGraphControl().addMouseListener(new MouseAdapter() {
			public void mouseReleased(MouseEvent e) {
				/*
				 * Object cell = graphComponent.getCellAt(e.getX(), e.getY()); if (cell != null)
	...
	public static void main(String[] args) {
		BTreeRender frame = new BTreeRender();
	...
	class BTIteratorImpl<K extends Comparable, V> implements BTIteratorIF<K, V> {
		...
		public boolean item(K key, V value) {
			...
			/*
			 * if (key.compareTo(30) == 0) { return false; }
			 */
			return true;
		}
	}
